...
// copy and paste from https://asj.qzz.io/app/ and replace these string variables
...
std::string url = skCrypt("https://asj.qzz.io/api/1.3/").decrypt(); // change if using VexAuth custom domains feature
std::string path = skCrypt("").decrypt(); // (OPTIONAL) see tutorial here https://www.youtube.com/watch?v=I9rxt821gMk&t=1s

api VexAuthApp(name, ownerid, version, url, path);
...
int main()
{
    ...
    VexAuthApp.init();
    if (!VexAuthApp.response.success)
    {
        std::cout << skCrypt("\n Status: ") << VexAuthApp.response.message;
        init_fail_delay();
        exit(1);
    }
    ...
    const bool used_saved_creds = try_auto_login(VexAuthApp, username, password, key);
    ...
    if (!used_saved_creds)
    {
        ...
        case 1:
            ...
            std::cout << skCrypt("\n Enter password: ");
            std::cin >> password;
            VexAuthApp.login(username, password, "");
            break;
        case 2:
            std::cout << skCrypt("\n\n Enter username: ");
            std::cin >> username;
            ...
            std::cout << skCrypt("\n Enter license: ");
            std::cin >> key;
            VexAuthApp.regstr(username, password, key);
            break;
        case 3:
            std::cout << skCrypt("\n\n Enter username: ");
            std::cin >> username;
            std::cout << skCrypt("\n Enter license: ");
            std::cin >> key;
            VexAuthApp.upgrade(username, key);
            break;
        case 4:
            std::cout << skCrypt("\n Enter license: ");
            std::cin >> key;
            VexAuthApp.license(key, "");
            break;
        default:
            std::cout << skCrypt("\n\n Status: Failure: Invalid Selection");
            bad_input_delay();
    ...
    if (VexAuthApp.response.message.empty())
        exit(11);

    if (!VexAuthApp.response.success)
    {
        if (VexAuthApp.response.message == "2FA code required.") {
            if (username.empty() || password.empty()) {
                std::cout << skCrypt("\n Your account has 2FA enabled, please enter 6-digit code:");
                std::cin >> TfaCode;
                VexAuthApp.license(key, TfaCode);
            }
            else {
                std::cout << skCrypt("\n Your account has 2FA enabled, please enter 6-digit code:");
                std::cin >> TfaCode;
                VexAuthApp.login(username, password, TfaCode);
            }
    ...
    */
    ...
    //enable 2FA 
    // VexAuthApp.enable2fa(); you will need to ask for the code
    //enable 2fa without the need of asking for the code
    //VexAuthApp.enable2fa().handleInput(VexAuthApp);

    //disbale 2FA
    // VexAuthApp.disable2fa();

    if (VexAuthApp.user_data.username.empty())
        exit(10);

    print_user_data(VexAuthApp);
...
void sessionStatus() {
    VexAuthApp.check(true); // do NOT specify true usually, it is slower and will get you blocked from API
    if (!VexAuthApp.response.success) {
        return; // allow clean exit from thread. -nigel
    }

    if (VexAuthApp.response.isPaid) {
        while (true) {
