...
void start_heartbeat(VexAuth::api* instance);
void heartbeat_thread(VexAuth::api* instance);
...
void VexAuth::api::init()
{
    ...
    if (response == XorStr("VexAuth_Invalid").c_str()) {
    ...
    if (signature.empty() || signatureTimestamp.empty()) { // used for debug
        ...
        MessageBoxA(0, "Missing signature headers in response", "VexAuth", MB_ICONERROR);
...
void VexAuth::api::login(std::string username, std::string password, std::string code)
{
    checkInit();
...
void VexAuth::api::chatget(std::string channel)
{
    checkInit();
...
bool VexAuth::api::chatsend(std::string message, std::string channel)
{
    checkInit();
...
void VexAuth::api::changeUsername(std::string newusername)
{
    checkInit();
...
VexAuth::api::Tfa& VexAuth::api::enable2fa(std::string code)
{
    checkInit();

   VexAuth::api::activate = true;
...
VexAuth::api::Tfa& VexAuth::api::disable2fa(std::string code)
{
    checkInit();
    
    VexAuth::api::activate = false;
...
void VexAuth::api::Tfa::QrCode() {
    auto qrcode = QrToPng("QRCode.png", 300, 3, VexAuth::api::Tfa::link, true, qrcodegen::QrCode::Ecc::MEDIUM);
    qrcode.writeToPNG();
}

VexAuth::api::Tfa& VexAuth::api::Tfa::handleInput(VexAuth::api& instance) {
...
void VexAuth::api::button(std::string button)
{
    checkInit();
...
void VexAuth::api::regstr(std::string username, std::string password, std::string key, std::string email) {
    checkInit();
    ScopeWipe wipe_user(username);
    ScopeWipe wipe_pass(password);
...
void VexAuth::api::upgrade(std::string username, std::string key) {
    checkInit();
    ScopeWipe wipe_user(username);
    ScopeWipe wipe_key(key);
...
void VexAuth::api::license(std::string key, std::string code) {
    checkInit();
    ScopeWipe wipe_key(key);
    ScopeWipe wipe_code(code);
...
void VexAuth::api::setvar(std::string var, std::string vardata) {
    checkInit();
...
std::string VexAuth::api::getvar(std::string var) {
    checkInit();
    ScopeWipe wipe_var(var);
...
void VexAuth::api::ban(std::string reason) {
    checkInit();
...
void VexAuth::api::check(bool check_paid) {
    checkInit();
...
std::string VexAuth::api::var(std::string varid) {
    checkInit();
...
void VexAuth::api::log(std::string message) {
    checkInit();
    ScopeWipe wipe_message(message);
...
std::vector<unsigned char> VexAuth::api::download(std::string fileid) {
    checkInit();
...
std::string VexAuth::api::webhook(std::string id, std::string params, std::string body, std::string contenttype)
{
    checkInit();
...
std::string VexAuth::api::fetchonline() 
{
    checkInit();
...
void VexAuth::api::forgot(std::string username, std::string email)
{
    checkInit();
    ScopeWipe wipe_user(username);
    ScopeWipe wipe_email(email);
...
void VexAuth::api::start_ban_monitor(int interval_seconds, bool check_session, std::function<void()> on_ban)
{
...
void VexAuth::api::stop_ban_monitor()
{
    ban_monitor_running_ = false;
...
bool VexAuth::api::ban_monitor_running() const
{
    return ban_monitor_running_.load();
}

bool VexAuth::api::ban_monitor_detected() const
{
    return ban_monitor_detected_.load();
}

std::string VexAuth::api::expiry_remaining(const std::string& expiry)
{
    if (expiry.empty())
        return "unknown";
    long long exp = 0;
...
void VexAuth::api::init_fail_delay()
{
    Sleep(kInitFailSleepMs);
}

void VexAuth::api::bad_input_delay()
{
    Sleep(kBadInputSleepMs);
}

void VexAuth::api::close_delay()
{
    Sleep(kCloseSleepMs);
}

bool VexAuth::api::lockout_active(const lockout_state& state)
{
    return std::chrono::steady_clock::now() < state.locked_until;
}

int VexAuth::api::lockout_remaining_ms(const lockout_state& state)
{
...
void VexAuth::api::record_login_fail(lockout_state& state, int max_attempts, int lock_seconds)
{
    if (max_attempts < 1)
        max_attempts = 1;
    if (lock_seconds < 1)
...
void VexAuth::api::reset_lockout(lockout_state& state)
{
    state.fails = 0;
...
int VerifyPayload(std::string signature, std::string timestamp, std::string body)
{
    ...
    if (timestamp.size() < 10 || timestamp.size() > 13) {
        MessageBoxA(0, "Signature verification failed (timestamp length)", "VexAuth", MB_ICONERROR);
        exit(2);
    }
    for (char c : timestamp) {
        if (c < '0' || c > '9') {
            MessageBoxA(0, "Signature verification failed (timestamp format)", "VexAuth", MB_ICONERROR);
            exit(2);
        }
    }
    ...
    try {
        ...
        MessageBoxA(0, "Signature verification failed (invalid timestamp)", "VexAuth", MB_ICONERROR);
        exit(2);
    }
    ...
    if (diff > 120) {
        std::cerr << "[ERROR] Timestamp too skewed (diff = "
            << diff << "s)\n";
        MessageBoxA(0, "Signature verification failed (timestamp skew)", "VexAuth", MB_ICONERROR);
        exit(3);
    }

    if (sodium_init() < 0) {
        std::cerr << "[ERROR] Failed to initialize libsodium\n";
        MessageBoxA(0, "Signature verification failed (libsodium init)", "VexAuth", MB_ICONERROR);
        exit(4);
    }
    ...
    if (signature.size() != 128) {
        MessageBoxA(0, "Signature verification failed (sig length)", "VexAuth", MB_ICONERROR);
        exit(5);
    }
    if (sodium_hex2bin(sig, sizeof(sig), signature.c_str(), signature.length(), NULL, NULL, NULL) != 0) {
        ...
        MessageBoxA(0, "Signature verification failed (invalid signature format)", "VexAuth", MB_ICONERROR);
        exit(5);
    }
    ...
    if (sodium_hex2bin(pk, sizeof(pk), API_PUBLIC_KEY.c_str(), API_PUBLIC_KEY.length(), NULL, NULL, NULL) != 0) {
        ...
        MessageBoxA(0, "Signature verification failed (invalid public key)", "VexAuth", MB_ICONERROR);
        exit(6);
    }
        ...
        pk) != 0)
    {
        ...
        MessageBoxA(0, "Signature verification failed (invalid signature)", "VexAuth", MB_ICONERROR);
        exit(7);
    }
...
{
    if (host_lower == "vexauth.win" || host_lower == "vexauth.cc" || host_lower == "api-worker.vexauth.win")
        return true;
    const std::string suffix = ".vexauth.win";
...
void VexAuth::api::setDebug(bool value) {
    VexAuth::api::debug = value;
}

std::string VexAuth::api::req(std::string data, const std::string& url) {
    signature.clear();
    ...
    // block loopback/private redirects for vexauth domains -nigel
    {
        std::string host_lower = host;
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "VexAuth");
...
void VexAuth::api::debugInfo(std::string data, std::string url, std::string response, std::string headers) {
    // output debug logs to C:\ProgramData\VexAuth\Debug\
    ...
    std::string VexAuthPath = path + "\\VexAuth";
    std::string logPath = VexAuthPath + "\\Debug\\" + filenameOnly.substr(0, filenameOnly.size() - 4);

    //basically loops until we have all the paths
    if (!std::filesystem::exists(VexAuthPath) || !std::filesystem::exists(VexAuthPath + "\\Debug") || !std::filesystem::exists(logPath)) {

        if (!std::filesystem::exists(VexAuthPath)) { std::filesystem::create_directory(VexAuthPath); }

        if (!std::filesystem::exists(VexAuthPath + "\\Debug")) { std::filesystem::create_directory(VexAuthPath + "\\Debug"); }
...
void checkInit() {
    if (!initialized) {
        error(XorStr("You need to run the VexAuthApp.init(); function before any other VexAuth functions"));
    }
