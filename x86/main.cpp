...
std::string url = skCrypt("https://asj.qzz.io/api/1.3/").decrypt(); // change if you're self-hosting
...
int main()
{
    ...
    VexAuthApp.set_allowed_hosts({ "vexauth.win" });
    // VexAuthApp.add_allowed_host("api.example.com"); // add custom domain if used
    // VexAuthApp.add_pinned_public_key("sha256//BASE64_SPKI_HASH"); // optional pin

    if (api::lockout_active(login_guard)) {
        std::cout << skCrypt("\n Status: Too many attempts. Try again in ")
                  << api::lockout_remaining_ms(login_guard) << skCrypt(" ms.");
        VexAuthApp.close_delay();
        return 0;
    }
